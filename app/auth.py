...
from .models import User as Userdb, Role, RolePermissions, Permission
from .schemas import UserPublic,TokenData
...
async def get_current_user(token: Annotated[str, Depends(oauth2_scheme)],db:SessionDep)->Userdb:
...
def assign_permissions_to_role(
        db:Session,
        role:Role,
        permission_names:list[PermissionsEnum]
):
    permissions_str=[p.value for p in permission_names]
    permissions=db.exec(select(Permission).where(Permission.name.in_(permissions_str))).all()
