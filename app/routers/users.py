...
from ..auth import get_current_active_user,get_password_hash,assign_role_to_user,require_permission,RoleEnum,PermissionsEnum
...
async def create_user(user:UserIn,db:SessionDep):
    existing_user=db.exec(select(Userdb).where(Userdb.username==user.username)).first()
    if existing_user:
...
@router.get('/users',response_model=list[UserPublic],dependencies=[Depends(require_permission(PermissionsEnum.VIEW_USER))])
...
@router.patch('/users/{user_id}/delete',dependencies=[Depends(require_permission(PermissionsEnum.DELETE_USER))])
...
# @router.patch("/users/{user_id}/role",response_model=User,dependencies=[Depends(require_role("admin"))])
...
#     role=session.exec(select(Role).where(Role.name==new_role.name)).first()
